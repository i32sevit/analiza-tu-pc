...
export default function AnalizaTuPcApp() {
  ...
  const ejecutarAnalisis = async (esManual = false) => {
    ...
    try {
      ...
      // Llamar a tu API
      const response = await axios.post(`${API_URL}/analizar`, datosAnalisis);
  ...
  const realizarAnalisisLocal = () => {
    const datos = datosManuales.cpuModel ? datosManuales : {
      cpuModel: "Intel Core i7-10700K",
      cpuSpeed: "3.8",
      cpuCores: "8",
      ram: "16",
    ...
    const resultadoLocal = generarAnalisisLocal(datos);
    setAnalisis(resultadoLocal);
    Alert.alert('ℹ️ Análisis Local', 'Se usó análisis local');
  };

  const generarAnalisisLocal = (datos) => {
    ...
    return {
      ...
      componentes: {
        cpu: {
          ...
          puntuacion: calcularPuntuacionCPU(datos),
          detalles: `${datos.cpuSpeed} GHz, ${datos.cpuCores} núcleos`
        },
        ram: {
          nombre: 'Memoria RAM',
          puntuacion: calcularPuntuacionRAM(datos),
          detalles: `${datos.ram} GB`
        },
        almacenamiento: {
          nombre: 'Almacenamiento',
          puntuacion: calcularPuntuacionAlmacenamiento(datos),
          detalles: datos.storageType
        },
        gpu: {
          nombre: datos.gpuModel,
          puntuacion: calcularPuntuacionGPU(datos),
          detalles: `${datos.gpuVram} GB VRAM`
        }
      },
      recomendaciones: generarRecomendaciones(puntuacionBase, datos)
    };
  };

  const calcularPuntuacionCPU = (datos) => {
    let score = 50;
    if (datos.cpuModel.includes('i9') || datos.cpuModel.includes('Ryzen 9')) score += 35;
    else if (datos.cpuModel.includes('i7') || datos.cpuModel.includes('Ryzen 7')) score += 25;
    else if (datos.cpuModel.includes('i5') || datos.cpuModel.includes('Ryzen 5')) score += 15;
    
    if (parseFloat(datos.cpuSpeed) > 4.0) score += 10;
    if (parseInt(datos.cpuCores) >= 8) score += 10;
    
    return Math.min(score, 100);
  };

  const calcularPuntuacionRAM = (datos) => {
    const ram = parseInt(datos.ram);
  ...
  const calcularPuntuacionAlmacenamiento = (datos) => {
    if (datos.storageType === 'NVMe') return 95;
    if (datos.storageType === 'SSD') return 80;
    return 50;
  };

  const calcularPuntuacionGPU = (datos) => {
    let score = 50;
    if (datos.gpuModel.includes('RTX 40') || datos.gpuModel.includes('RX 7900')) score += 40;
    else if (datos.gpuModel.includes('RTX 30') || datos.gpuModel.includes('RX 6000')) score += 30;
    else if (datos.gpuModel.includes('RTX 20') || datos.gpuModel.includes('RX 5000')) score += 20;
    
    if (parseFloat(datos.gpuVram) >= 8) score += 10;
    else if (parseFloat(datos.gpuVram) >= 4) score += 5;
    
    return Math.min(score, 100);
  };

  const generarRecomendaciones = (score, datos) => {
    const recomendaciones = [];
    
    if (score < 60) {
      recomendaciones.push('Considera actualizar tu procesador para mejorar el rendimiento general');
      recomendaciones.push('Aumentar la memoria RAM mejorará significativamente la multitarea');
    }
    
    if (parseInt(datos.ram) < 8) {
      recomendaciones.push('Se recomienda al menos 8GB de RAM para un rendimiento óptimo');
    }
    
    if (datos.storageType === 'HDD') {
      recomendaciones.push('Cambiar a un SSD mejorará drásticamente los tiempos de carga');
    }
    
    if (parseFloat(datos.gpuVram) < 4 && score >= 70) {
      recomendaciones.push('Una GPU con más VRAM mejorará el rendimiento en juegos y aplicaciones gráficas');
    }
    
    if (score >= 80) {
      recomendaciones.push('Tu sistema está bien equilibrado para la mayoría de tareas');
      recomendaciones.push('Considera optimizaciones de software para maximizar el rendimiento');
    }
    
    recomendaciones.push('Mantén tus controladores actualizados para el mejor rendimiento');
    recomendaciones.push('Realiza mantenimiento regular del sistema para mantener el rendimiento');
    ...
    return (
      ...
      >
          ...
          <View style={styles.scoreCircle}>
            <ProgressChart
              ...
              chartConfig={{
                ...
                decimalPlaces: 0,
                color: (opacity = 1) => obtenerColorPuntuacion(analisis.main_score),
              }}
              hideLegend={true}
            />
        ...
        <View style={styles.chartContainer}>
          <Text style={styles.chartTitle}>Rendimiento por Perfil</Text>
          <BarChart
            data={{
              labels: Object.keys(analisis.scores),
              datasets: [{
                data: Object.values(analisis.scores).map(score => score)
              }]
            }}
            ...
            chartConfig={{
              backgroundColor: 'transparent',
              backgroundGradientFrom: COLORS.dark3,
        ...
        <View style={styles.profileGrid}>
          {Object.entries(analisis.scores).map(([perfil, puntuacion]) => (
            <TouchableOpacity
              ...
              onPress={() => {
                setComponenteSeleccionado({ tipo: 'perfil', nombre: perfil, puntuacion });
                setModalVisible(true);
              }}
            >
              <Text style={styles.profileName}>{perfil}</Text>
              <Text style={[styles.profileScore, { color: obtenerColorPuntuacion(puntuacion) }]}>
                {puntuacion}%
              </Text>
              <View style={styles.profileBar}>
                <View 
                  style={[
                    styles.profileFill,
                    { width: `${puntuacion}%`, backgroundColor: obtenerColorPuntuacion(puntuacion) }
                  ]} 
                />
              </View>
              <View style={styles.ratingStars}>
                {obtenerEstrellas(puntuacion)}
              </View>
            </TouchableOpacity>
          ))}
        </View>
